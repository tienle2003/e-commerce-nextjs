...
import { useCategoryBySlugQuery } from "@/hooks/api/category.api";
...
const CategoryPage = () => {
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [selectedColor, priceRange, sortType, slug]);
  ...
  useEffect(() => {
    ...
    const newUrl = `/danh-muc-san-pham/${slug}?${params.toString()}`;
    if (newUrl !== `/danh-muc-san-pham/${slug}?${searchParams.toString()}`) {
      router.replace(newUrl);
    }
  }, [filters, currentPage, slug]);

  const { data: categoryData } = useCategoryBySlugQuery(slug!);
  ...
  const categoryName = categoryData?.data?.name || slug;
  const searchItems =
      ...
      <div className="flex flex-col md:flex-row gap-6 md:gap-10">
        ...
        <main className="flex-1">
          <div className="flex flex-col sm:flex-row items-center sm:items-center gap-2 sm:gap-3 mb-4 sm:mb-6">
            <div className="text-lg font-bold sm:text-xl leading-6 border-r border-gray-500 pr-4">
              {categoryName}
            </div>
